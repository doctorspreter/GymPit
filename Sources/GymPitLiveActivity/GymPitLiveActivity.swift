                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(state.exerciseName)
                        ...
                        HStack(spacing: 8) {
                            ...
                            LiveElapsedText(state: state, label: language.ui("Gesamtzeit"), font: .caption2.monospacedDigit())
                                .foregroundStyle(.secondary)
                                .lineLimit(1)
                                .layoutPriority(1)
                        }
        ...
        TimelineView(.periodic(from: .now, by: 1)) { timeline in
            if isResting(at: timeline.date) {
                Button(intent: SkipWorkoutRestIntent(exerciseIDString: state.exerciseIDString)) {
                    buttonLabel(compact ? language.ui("Überspringen") : language.ui("Pause überspringen"), systemImage: "forward.fill", tint: .orange)
                }
                .buttonStyle(.plain)
            } else {
                Button(intent: CompleteNextWorkoutSetIntent(exerciseIDString: state.exerciseIDString)) {
                    buttonLabel(compact ? language.ui("Satz") : language.ui("Satz erledigt"), systemImage: "checkmark", tint: .blue)
                }
                .buttonStyle(.plain)
            }
        }
            ...
            VStack(alignment: .leading, spacing: 2) {
                if let restEndDate = state.restEndDate, restEndDate > timeline.date {
                    Text(language.ui("Pause"))
                        ...
                        .foregroundStyle(.orange)
                    Text("\(language.ui("Nächster")) \(state.setText)")
                        .font(.headline.weight(.semibold))
                        .foregroundStyle(.primary)
                        .lineLimit(1)
                        .minimumScaleFactor(0.82)
