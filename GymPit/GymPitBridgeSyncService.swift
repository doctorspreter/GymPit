...
enum GymPitBridgeSettings {
    ...
    static let apiTokenKey = "gymPitBridgeApiToken"
    static let otpCodeKey = "gymPitBridgeOtpCode"
    static let sessionTokenKey = "gymPitBridgeSessionToken"
    static let sessionExpiresAtKey = "gymPitBridgeSessionExpiresAt"
...
private struct GymPitBridgeSessionCreatePayload: Encodable {
    let username: String
    let apiToken: String
    let otpCode: String
    let deviceName: String
...
final class GymPitBridgeSyncService {
    ...
    private init() {
        ...
        if defaults.string(forKey: GymPitBridgeSettings.localPortKey)?.isEmpty != false {
            defaults.set("8088", forKey: GymPitBridgeSettings.localPortKey)
        }
        GymPitBridgeKeychainStore.delete(GymPitBridgeSettings.otpCodeKey)
    }

    var hasSession: Bool {
        !Self.trimmedKeychainValue(for: GymPitBridgeSettings.sessionTokenKey).isEmpty
    }

    var sessionExpiresAtText: String {
        defaults.string(forKey: GymPitBridgeSettings.sessionExpiresAtKey) ?? ""
    }

    @discardableResult
    func connect(otpCode: String) async throws -> GymPitBridgeSession {
        let apiToken = Self.trimmedKeychainValue(for: GymPitBridgeSettings.apiTokenKey)
        let username = defaults.string(forKey: GymPitBridgeSettings.usernameKey)?
            .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !username.isEmpty else {
            throw GymPitBridgeSyncError.serverMessage("Healthpit-Benutzername fehlt.")
        }
        guard !apiToken.isEmpty else {
            throw GymPitBridgeSyncError.missingToken
        }
        ...
        let payload = GymPitBridgeSessionCreatePayload(
            username: username,
            apiToken: apiToken,
        ...
        let session = try decoder.decode(GymPitBridgeSession.self, from: data)
        GymPitBridgeKeychainStore.set(session.sessionToken, for: GymPitBridgeSettings.sessionTokenKey)
        GymPitBridgeKeychainStore.delete(GymPitBridgeSettings.otpCodeKey)
        defaults.set(session.expiresAt, forKey: GymPitBridgeSettings.sessionExpiresAtKey)
        return session
    }

    func disconnect() {
        GymPitBridgeKeychainStore.delete(GymPitBridgeSettings.sessionTokenKey)
        GymPitBridgeKeychainStore.delete(GymPitBridgeSettings.otpCodeKey)
        defaults.removeObject(forKey: GymPitBridgeSettings.sessionExpiresAtKey)
    }
    ...
    func delete(workoutID: UUID) async throws {
        ...
        endpoint.append(path: "v1/workouts/imports/\(workoutID.uuidString)")
    ...
    private func validate(
        ...
        let statusCode = (response as? HTTPURLResponse)?.statusCode ?? 0
        guard (200..<300).contains(statusCode) else {
            if statusCode == 401, credentials.usesSessionToken {
                disconnect()
                throw GymPitBridgeSyncError.serverMessage("Healthpit-Verbindung abgelaufen, bitte neu verbinden.")
            }
            if let message = Self.bridgeErrorMessage(from: data, statusCode: statusCode) {
                throw GymPitBridgeSyncError.serverMessage(message)
            }
    ...
    private func bridgeCredentials() async throws -> GymPitBridgeCredentials {
        let sessionToken = Self.trimmedKeychainValue(for: GymPitBridgeSettings.sessionTokenKey)
        let apiToken = Self.trimmedKeychainValue(for: GymPitBridgeSettings.apiTokenKey)
        let username = defaults.string(forKey: GymPitBridgeSettings.usernameKey)?
            .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        ...
        guard !sessionToken.isEmpty || !apiToken.isEmpty else {
            throw GymPitBridgeSyncError.missingToken
        }
        ...
        return GymPitBridgeCredentials(
            baseURL: baseURL,
            username: username,
            authToken: sessionToken.isEmpty ? apiToken : sessionToken,
            usesSessionToken: !sessionToken.isEmpty,
    ...
    static func configuredBaseURL(defaults: UserDefaults = .standard) async throws -> URL {
        ...
        let localPort = defaults.string(forKey: GymPitBridgeSettings.localPortKey)?
            .trimmingCharacters(in: .whitespacesAndNewlines) ?? "8088"
    ...
    private static func isBridgeReachable(at baseURL: URL) async -> Bool {
        ...
        do {
            ...
            if object["status"] as? String == "ok" { return true }
    ...
    private static func bridgeErrorMessage(from data: Data, statusCode: Int) -> String? {
        ...
        if detail == "Invalid bridge token" {
            return "Healthpit API Token stimmt nicht. Bitte Token in App und Healthpit vergleichen."
        }
        if detail == "Invalid bridge username" {
            return "Healthpit Benutzername stimmt nicht. Bitte Benutzername in App und Healthpit vergleichen."
        }
        if detail == "Invalid bridge OTP" {
            return "Healthpit OTP stimmt nicht. Bitte aktuellen 6-stelligen Code eingeben."
        }
