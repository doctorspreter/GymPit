...
final class WorkoutStore: ObservableObject {
    ...
    func updateSet(_ set: ExerciseSet, in exercise: Exercise, type: WorkoutSetType, reps: Int, weight: Double, rpe: Int?, isLogged: Bool) {
        ...
        let wasLogged = plan.exercises[exerciseIndex].sets[setIndex].isLogged
        plan.exercises[exerciseIndex].sets[setIndex].type = type
        plan.exercises[exerciseIndex].sets[setIndex].reps = reps
        plan.exercises[exerciseIndex].sets[setIndex].weight = normalizedWeight(weight)
        plan.exercises[exerciseIndex].sets[setIndex].rpe = rpe
        plan.exercises[exerciseIndex].sets[setIndex].isLogged = isLogged
    ...
    func exportAllHistoricSessionsToHealth() {
        ...
        healthExportStatus = "Übertrage \(sessions.count) alte Workouts..."
        exportHistoricSessions(sessions, total: sessions.count, completed: 0, failed: 0)
    }
    ...
    func startRestTimer(seconds: Int? = nil) {
        restRemainingSeconds = max(0, seconds ?? plan.profile.defaultRestSeconds)
    ...
    func addRestTime(_ seconds: Int) {
        ...
        restRemainingSeconds = max(0, Int(restEndDate.timeIntervalSinceNow.rounded(.up)))
    ...
    private func exportHistoricSessions(_ sessions: [WorkoutSession], total: Int, completed: Int, failed: Int) {
        guard let session = sessions.first else {
            if failed == 0 {
                healthExportStatus = "Alle alten Workouts übertragen (\(completed))"
            } else {
                healthExportStatus = "\(completed) alte Workouts übertragen, \(failed) Fehler"
            }
            return
        }
        ...
        WorkoutHealthExporter.shared.save(session) { [weak self] result in
            Task { @MainActor in
                ...
                let nextCompleted = completed + (didSucceed ? 1 : 0)
                let nextFailed = failed + (didSucceed ? 0 : 1)
                self?.healthExportStatus = "Alte Workouts: \(nextCompleted + nextFailed)/\(total) verarbeitet"
                self?.exportHistoricSessions(remaining, total: total, completed: nextCompleted, failed: nextFailed)
            }
        }
    }
    ...
    private func nextSetText(for exercise: Exercise) -> String {
        guard let index = nextOpenSetIndex(for: exercise) else {
            return "alle Sätze erledigt"
        }
        return "Satz \(index + 1)/\(exercise.sets.count)"
    }
    ...
    private func restNotificationBody(for exercise: Exercise) -> String {
        ...
        return "\(exercise.localizedName(language: AppLanguage.current)): \(setText)\(weightText) bereit · Training \(trainingMinutes) min"
    }
