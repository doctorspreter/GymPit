...
private struct SettingsView: View {
    ...
    @AppStorage(GymPitBridgeSettings.localPortKey) private var bridgeLocalPort = "8088"
    ...
    @State private var bridgeApiToken = ""
    @State private var bridgeOtpCode = ""
    ...
    var body: some View {
        NavigationStack {
            ...
            .onChange(of: bridgeApiToken) { _, newValue in
                GymPitBridgeKeychainStore.set(newValue, for: GymPitBridgeSettings.apiTokenKey)
            ...
            .alert("CSV", isPresented: $isShowingCSVAlert) {
        ...
        Form {
            Section(appLanguage.ui("Daten")) {
                ...
                Button {
                    ...
                    Label(appLanguage.ui("Trainings exportieren"), systemImage: "square.and.arrow.up")
                }
                ...
                Button {
                    ...
                    Label(appLanguage.ui("Trainings importieren"), systemImage: "square.and.arrow.down")
                }
            ...
            Section(appLanguage.ui("Healthpit")) {
                TextField(appLanguage.ui("Lokale Verbindung"), text: $bridgeLocalHost)
                ...
                TextField(appLanguage.ui("Externe Verbindung (optional)"), text: $bridgeBaseURL)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                    .keyboardType(.URL)

                TextField(appLanguage.ui("Benutzername"), text: $bridgeUsername)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()

                SecureField(appLanguage.ui("API Token"), text: $bridgeApiToken)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()

                SecureField(appLanguage.ui("OTP Code zum Verbinden"), text: $bridgeOtpCode)
                    .keyboardType(.numberPad)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()

                Text(appLanguage.ui("Die externe Verbindung ist nur nötig, wenn die lokale Bridge nicht erreichbar ist. Den OTP-Code nur eintragen, wenn 2FA in der Bridge aktiv ist."))
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                ...
                .disabled(
                    ...
                    bridgeApiToken.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                )
    ...
    private func syncState() {
        ...
        bridgeApiToken = GymPitBridgeKeychainStore.string(for: GymPitBridgeSettings.apiTokenKey)
        bridgeOtpCode = ""
        refreshBridgeConnectionStatus()
    }

    private func connectBridge() {
        let otpCode = normalizedOTPCode(bridgeOtpCode)
        isConnectingBridge = true
        bridgeConnectionStatus = appLanguage.ui("Healthpit verbindet...")
        Task {
            do {
                let session = try await GymPitBridgeSyncService.shared.connect(otpCode: otpCode)
                await MainActor.run {
                    bridgeOtpCode = ""
                    isConnectingBridge = false
                    isBridgeConnected = true
                    bridgeConnectionStatus = "\(appLanguage.ui("Healthpit verbunden bis")) \(session.expiresAt)"
                }
            } catch {
                await MainActor.run {
                    isConnectingBridge = false
    ...
    private func refreshBridgeConnectionStatus() {
        ...
        if service.hasSession {
            ...
            bridgeConnectionStatus = expiresAt.isEmpty
                ? appLanguage.ui("Healthpit ist verbunden")
                : "\(appLanguage.ui("Healthpit verbunden bis")) \(expiresAt)"
        } else {
    ...
    private func normalizedOTPCode(_ value: String) -> String {
        value.filter(\.isNumber).prefix(6).map(String.init).joined()
    }
    ...
    private func processWorkoutImport(_ result: Result<URL, Error>) async {
        ...
        do {
            ...
            let sessions = try await Self.gymPitSessions(from: data)
            let summary = store.importHistorySessions(sessions)
            showCSVAlert("\(summary.imported) importiert, \(summary.skipped) übersprungen")
        } catch {
    ...
    private static func gymPitSessions(from data: Data) async throws -> [WorkoutSession] {
        try await Task.detached(priority: .userInitiated) {
            try WorkoutCSVCodec.importSessions(from: data)
        }.value
    }

    private func exportCSV() {
        do {
            let fileName = "gympit-trainings-\(Self.csvFileDateFormatter.string(from: Date())).csv"
            let url = FileManager.default.temporaryDirectory.appendingPathComponent(fileName)
            try Data(store.exportHistoryCSV().utf8).write(to: url, options: .atomic)
...
private struct ManualWorkoutDraftSet: Identifiable, Equatable {
    let id = UUID()
    var type: WorkoutSetType = .normal
    var reps: Int
...
private struct ManualWorkoutDraftExercise: Identifiable, Equatable {
    ...
    init(item: ExerciseCatalogItem) {
        ...
        sets = (0..<max(1, item.defaultSets)).map { _ in
            ManualWorkoutDraftSet(reps: item.defaultReps, weightText: formattedWeightInput(item.defaultWeight))
        }
    }
    ...
    func sessionExercise(weightUnit: WeightUnit) -> WorkoutSessionExercise {
        WorkoutSessionExercise(
            ...
            sets: sets.map { $0.sessionSet(weightUnit: weightUnit) }
        )
    }
}
...
private struct ManualWorkoutEntryView: View {
    ...
    @State private var planName = "Training"
    @State private var performedAt = Date()
    @State private var durationMinutes = 60
    @State private var caloriesText = ""
    @State private var notes = ""
    @State private var selectedCatalogID = ExerciseCatalog.all.first?.id ?? ""
    ...
    var body: some View {
        Form {
            ...
            Section(appLanguage.ui("Übung hinzufügen")) {
                Picker(appLanguage.ui("Übung"), selection: $selectedCatalogID) {
                    ForEach(ExerciseCatalog.all) { item in
                        Text(item.localizedName(language: appLanguage)).tag(item.id)
                    }
                }
    ...
    private func addSelectedExercise() {
        guard let item = ExerciseCatalog.item(for: selectedCatalogID) ?? ExerciseCatalog.all.first else { return }
        exercises.append(ManualWorkoutDraftExercise(item: item))
    }

    private func save() {
        let parsedCalories = Int(caloriesText.trimmingCharacters(in: .whitespacesAndNewlines))
        let sessionExercises = exercises.map { $0.sessionExercise(weightUnit: weightUnit) }
...
private struct ManualWorkoutExerciseEditor: View {
        ...
        Section {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(exercise.localizedName(language: appLanguage))
                        .font(.subheadline.weight(.semibold))
                    Text(exercise.category.localizedName(language: appLanguage))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
    ...
    private func addSet() {
        let template = exercise.sets.last ?? ManualWorkoutDraftSet(reps: 12, weightText: formattedWeightInput(0))
        exercise.sets.append(ManualWorkoutDraftSet(type: template.type, reps: template.reps, weightText: template.weightText, rpe: template.rpe))
    }
}
...
private struct AddExerciseView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(appLanguage.ui("Standardgeräte"))
    ...
    var body: some View {
        Form {
            ...
            Section(appLanguage.ui("Plan")) {
                TextField("Ziel, z. B. 3 x 12", text: $target)
        ...
        Form {
            ...
            Section(appLanguage.ui("Optionen")) {
                ...
                Picker("Superset", selection: $supersetGroup) {
                    ...
                    Text("Superset A").tag(1)
                    Text("Superset B").tag(2)
                    Text("Superset C").tag(3)
                }
            }

            Section(appLanguage.ui("Notizen")) {
                TextField("Technik, Ziel oder Erinnerung", text: $notes, axis: .vertical)
                    .lineLimit(2...6)
            }
        ...
        HStack(spacing: 8) {
            VStack(alignment: .leading, spacing: 2) {
                Text(store.plan.name)
                    ...
                    .lineLimit(1)
                Text("\(store.plan.openExercises.count) \(appLanguage.ui("offen")), \(store.plan.completedExercises.count) \(appLanguage.ui("erledigt")) · \(store.plan.progressSummary)")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                if store.plan.isWorkoutStarted {
...
private struct TrainingSummaryCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 8) {
                StatTile(title: appLanguage.ui("Volumen"), value: totalVolume.formattedWeight(unit: weightUnit), icon: "scalemass")
                StatTile(title: appLanguage.ui("Sätze"), value: "\(totalSets)", icon: "checkmark.circle")
                StatTile(title: appLanguage.ui("Zeit"), value: "\(durationMinutes) min", icon: "timer")
            }

            HStack(spacing: 8) {
                StatTile(title: appLanguage.ui("Kalorien"), value: "\(calories)", icon: "flame.fill")
                StatTile(title: appLanguage.ui("Rekorde"), value: "\(recordCount)", icon: "trophy.fill")
                StatTile(title: appLanguage.ui("Übungen"), value: "\(exerciseCount)", icon: "figure.strengthtraining.traditional")
            }
    ...
    var body: some View {
        Form {
            Section(appLanguage.ui("Übungsnotiz")) {
                TextField("Technik, Gefühl, Schmerzen, Ziel", text: $notes, axis: .vertical)
                    .lineLimit(5...10)
            }
...
private enum ExerciseImageLoader {
    static func image(named name: String) -> UIImage? {
        image(named: name, subdirectory: "ExerciseImages")
    }
    ...
    private static func image(named name: String, subdirectory: String) -> UIImage? {
        if let catalogImage = UIImage(named: name) {
            return catalogImage
        }
            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if let group = exercise.supersetGroup {
                        Text("Superset \(groupLabel(group))")
                            .font(.caption.weight(.semibold))
                            .foregroundStyle(Color.accentColor)
                    }
                }
...
private struct DeviceManagementView: View {
    ...
    var body: some View {
        List {
            ...
            if mode == .devices {
                ...
                ForEach(store.routines) { routine in
                    let exercises = filteredExercises(in: routine)
                    if !exercises.isEmpty {
                ...
                ForEach(DeviceCategory.allCases) { category in
                    let items = filteredCatalogItems(for: category)
                    if !items.isEmpty {
    ...
    private func filteredExercises(in routine: WorkoutPlan) -> [Exercise] {
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !query.isEmpty else { return routine.exercises }
    ...
    private func filteredCatalogItems(for category: DeviceCategory) -> [ExerciseCatalogItem] {
        let items = ExerciseCatalog.all.filter { $0.category == category }
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private var filteredCustomExercises: [Exercise] {
        ...
        let exercises = store.routines.flatMap(\.exercises).filter { exercise in
            exercise.isCustom && seenCatalogIDs.insert(exercise.catalogID).inserted
        }
    ...
    private var filteredCustomDevices: [Exercise] {
        filteredCustomExercises.filter { $0.device.visibleMachineName != nil }
    }
...
private struct DeviceExerciseCreatorView: View {
    ...
    @State private var settings = DeviceSettings(machineName: "", seat: "", backrest: "", handle: "", range: "", notes: "")
...
private struct ExerciseBadges: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            if let group = exercise.supersetGroup {
                Label("Superset \(groupLabel(group))", systemImage: "link")
                    .foregroundStyle(Color.accentColor)
            }
